...
class TumblrAppModel extends AppModel {
	...
	public function get_content($options) {
		...
		$this->duration = (isset($options['duration'])) ? Cache::set(array('duration' => $options['duration']),'CacheTumblrFeed') : false;
		...
		//clear cache data
		if ($this->delete_cache) Cache::delete($this->cache_key,'CacheTumblrFeed');
		
		//get cached data
		if (!$this->no_cache) {
			$cached =  Cache::read($this->cache_key,'CacheTumblrFeed');
			...
			if ($cached) {
				...
				if ($fresh_feed) {
					Cache::write($this->cache_key, $fresh_feed,'CacheTumblrFeed'); 
					...
					Cache::set(array('duration' => '+30 days'),'CacheTumblrFeed');
					$old_cache = Cache::read($this->cache_key,'CacheTumblrFeed');
					if ($old_cache) {
	...
	private function get_feed_url($feed = null){
		if (!empty($this->user_id)) {
			$feed	= "http://api.tumblr.com/v2/blog/{$this->user_id}.tumblr.com/posts?api_key=JJJi1al63EOYVgVuaB3RBd83tslGo4uX12sUQTSmrLLJ9m1c3d";
		}
		return $feed;
	}
	...
	private function decoded($feed,$results = null) {
